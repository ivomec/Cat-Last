...
document.addEventListener('DOMContentLoaded', () => {
    // 1. 기존 치과병원 탭 콘텐츠 및 계산기 초기화
    try {
        const hospitalData = getHospitalData(); // 데이터 가져오기
        populateAllTabs(hospitalData);
        initCalculator();
    } catch (error) {
        console.error('기존 탭 콘텐츠 처리 중 오류 발생:', error);
    }
    
    // 2. 공통 페이지 네비게이션(탭) 기능 설정
    setupPageNavigation();

    // 3. 계산기/보호자용 탭의 내보내기 버튼 리스너 추가
    ...
    // 4. [신규] 마취 대시보드 기능 초기화
    try {
        setupAnesthesiaDashboard();
    } catch(error) {
        console.error('마취 대시보드 초기화 중 오류:', error);
    }
    
    // 5. [신규] 보호자 교육 대시보드 기능 초기화
    try {
        setupEducationDashboard();
    } catch (error) {
        console.error('보호자 교육 대시보드 초기화 중 오류:', error);
    }
});
...
### **2. 기존 치과 탭 콘텐츠 생성 로직**
병원 소개, 수술 과정, 비용 안내 등 기존 탭들의 내용을 채우는 부분입니다.
...
### **3. 치과 치료비 계산기 로직**
치과 치료비 계산기, 예상 비용 안내, 보호자용 리포트 생성 등과 관련된 기능입니다.
...
function initCalculator() {
    ...
    function populateAdditionalTreatments() {
        const leftContainer = page.querySelector('#additional-treatments-left');
        const rightContainer = page.querySelector('#additional-treatments-right');
...
### **4. 마취 통합 대시보드 로직**
'마취 AI' 탭의 계산, 탭 전환, 이미지 저장 등 모든 기능을 담당합니다.

```javascript
// ===================================================================================
// ====================== [ 4. 고양이 마취 통합 대시보드 스크립트 ] =====================
// ===================================================================================
function setupAnesthesiaDashboard() {
    const anesthesiaTab = document.getElementById('content-anesthesia');
    if (!anesthesiaTab) return;
    ...
    function openAnesthesiaTab(evt, tabName) {
        anesthesiaTab.querySelectorAll(".tab-content").forEach(tc => tc.style.display = "none");
        anesthesiaTab.querySelectorAll(".tab-button").forEach(tl => tl.classList.remove("active"));
        const targetTab = anesthesiaTab.querySelector(`#${tabName}`);
        if(targetTab) targetTab.style.display = "block";
        if(evt.currentTarget) evt.currentTarget.classList.add("active");
    }

    function calculateAnesthesiaAll() {
        updateCatTubeDisplay();
        const weightInput = anesthesiaTab.querySelector('#weight');
        const weight = (weightInput && weightInput.value) ? parseFloat(weightInput.value) : 0;
        
        const weightInputTube = anesthesiaTab.querySelector('#weight-input');
        if (weightInputTube) {
            weightInputTube.value = weight > 0 ? weight : '';
            calculateWeightSize();
        }
    ...
    function populatePrepTab(weight) {
        const status = anesthesiaTab.querySelector('#patient_status').value;
        const isChill = anesthesiaTab.querySelector('#chill_protocol').value === 'yes';
        ...
        anesthesiaTab.querySelector('#pre_op_drugs_result_cat').innerHTML = `
    ...
    function populateDischargeTab(weight) {
        ...
        let content = '';
        const vetrocamInput = anesthesiaTab.querySelector('#vetrocam_days_cat');
        const gabaAInput = anesthesiaTab.querySelector('#gabapentin_dose_cat_a');
        const gabaBInput = anesthesiaTab.querySelector('#gabapentin_dose_cat_b');
        
        if (renalStatus === 'healthy') {
            const vetrocamDays = (vetrocamInput && vetrocamInput.value) ? parseInt(vetrocamInput.value) : 3;
            const gabapentinDoseA = (gabaAInput && gabaAInput.value) ? parseFloat(gabaAInput.value) : 5;
            ...
            content = `<div id="discharge_gold_cat"><h3 class="font-bold text-lg text-green-700 mb-2">시나리오 1: 종합 처방 (신기능 정상)</h3><div class="p-4 bg-green-50 rounded-lg space-y-2"><div><label class="font-semibold text-sm">베트로캄 처방일: <input type="number" id="vetrocam_days_cat" value="${vetrocamDays}" class="w-16 p-0.5 border rounded text-center"></label></div><p><strong>베트로캄(액상, 1일 1회):</strong> 총 <span class="result-value">${totalVetrocamDoseMl.toFixed(2)} mL</span></p><hr><div><label class="font-semibold text-sm">가바펜틴 용량(mg/kg): <input type="number" id="gabapentin_dose_cat_a" value="${gabapentinDoseA}" class="w-16 p-0.5 border rounded text-center"></label></div><div class="text-sm p-1 bg-green-100 rounded">${getPillCount(gabapentinDoseA * weight, 2, pillStrengths_cat.gabapentin, generalDays)}</div><hr><p class="font-semibold text-sm">항생제/위장보호제는 동일</p></div></div>`;
        } else {
             const gabapentinDoseB = (gabaBInput && gabaBInput.value) ? parseFloat(gabaBInput.value) : 10;
             content = `<div id="discharge_alt_cat"><h3 class="font-bold text-lg text-orange-700 mb-2">시나리오 2: NSAID 제외 처방 (신기능 저하)</h3><div class="info-box mb-2 text-xs"><p class="font-bold text-red-600">NSAIDs 절대 금기!</p></div><div class="p-4 bg-orange-50 rounded-lg space-y-2"><div><label class="font-semibold text-sm">가바펜틴 용량(mg/kg): <input type="number" id="gabapentin_dose_cat_b" value="${gabapentinDoseB}" class="w-16 p-0.5 border rounded text-center"></label></div><div class="text-sm p-1 bg-orange-100 rounded">${getPillCount(gabapentinDoseB * weight, 2, pillStrengths_cat.gabapentin, generalDays)}</div><hr><p class="font-semibold text-sm">항생제/위장보호제는 동일</p></div></div>`;
        }
        anesthesiaTab.querySelector('#discharge_cat').innerHTML = content;
        anesthesiaTab.querySelector('#prescription_days_cat').oninput = calculateAnesthesiaAll;
        anesthesiaTab.querySelector('#vetrocam_days_cat')?.addEventListener('input', calculateAnesthesiaAll);
        anesthesiaTab.querySelector('#gabapentin_dose_cat_a')?.addEventListener('input', calculateAnesthesiaAll);
        anesthesiaTab.querySelector('#gabapentin_dose_cat_b')?.addEventListener('input', calculateAnesthesiaAll);
    }

    function populateNorspanProtocol(weight) {
        let patchRecommendation = weight <= 3.0 ? "5 mcg/h 1매" : weight <= 6.0 ? "10 mcg/h 1매" : "20 mcg/h 1매";
        anesthesiaTab.querySelector('#norspan_protocol_content').innerHTML = `<h4 class="font-bold text-gray-800 mb-2">1. 적용 대상 및 금기</h4><p><strong>적용:</strong> 고양이 수술 후 통증, 만성 통증(관절염, 종양), CKD 환자 통증 관리</p><p class="text-red-600"><strong>금기:</strong> 개, 급성 통증 단독 치료, 피부 질환 부위, 심각한 간부전</p><hr class="my-3"><h4 class="font-bold text-gray-800 mb-2">2. 용량 선택</h4><div class="p-2 bg-blue-50 rounded text-center font-bold text-blue-800">${weight.toFixed(1)}kg 환자 추천: ${patchRecommendation}</div><p class="text-red-600 font-bold text-sm mt-2">🚨 경고: 패치는 절대 자르지 마십시오!</p><hr class="my-3"><h4 class="font-bold text-gray-800 mb-2">3. 핵심 안전 수칙</h4><p class="text-red-600">🔥 패치 부착 환자에게 전기장판, 핫팩 등 모든 외부 열원 사용을 절대 금지해야 함을 보호자에게 반드시 교육!</p>`;
    }
    
    function calculateNorspanRemovalDate() {
        const handoutTab = anesthesiaTab.querySelector('#norspanHandoutTab');
        if (!handoutTab) return;
        const dateInput = handoutTab.querySelector('#attachDate')?.value;
        const timeInput = handoutTab.querySelector('#attachTime')?.value;
        const removalInfoDiv = handoutTab.querySelector('#removalInfo');
        if (!dateInput || !timeInput || !removalInfoDiv) { 
            if(removalInfoDiv) removalInfoDiv.innerHTML = '<p class="font-bold text-yellow-900">날짜와 시간을 입력해주세요.</p>'; 
            return;
        }
    ...
    function saveAnesthesiaImage(elementId, suffix) {
        const captureElement = anesthesiaTab.querySelector(`#${elementId}`);
        const patientName = anesthesiaTab.querySelector('#patientName')?.value || '환자';
    ...
    function exportPrepSheet() {
        const captureElement = anesthesiaTab.querySelector('#prepTab');
        const weight = anesthesiaTab.querySelector('#weight').value || '체중미입력';
        const patientName = anesthesiaTab.querySelector('#patientName')?.value || '환자';
    ...
    function calculateWeightSize() {
        const weightInput = anesthesiaTab.querySelector('#weight-input');
        const resultContainer = anesthesiaTab.querySelector('#result-container-weight');
        const resultText = anesthesiaTab.querySelector('#result-text-weight');
        if(!weightInput || !resultContainer || !resultText) return;
        const weight = parseFloat(weightInput.value);
        if (isNaN(weight) || weight <= 0) { resultContainer.classList.add('hidden'); return; }
        let recommendedSize = '4.5 이상';
        for (const guide of weightSizeGuideCat) { if (weight <= guide.weight) { recommendedSize = guide.size; break; } }
        resultText.textContent = recommendedSize;
        resultContainer.classList.remove('hidden');
    }

    function calculateTracheaSize() {
        const tracheaInput = anesthesiaTab.querySelector('#trachea-input');
        const resultContainer = anesthesiaTab.querySelector('#result-container-trachea');
        const resultText = anesthesiaTab.querySelector('#result-text-trachea');
        if(!tracheaInput || !resultContainer || !resultText) return;
        const diameter = parseFloat(tracheaInput.value);
        if (isNaN(diameter) || diameter <= 0) { resultContainer.classList.add('hidden'); return; }
        let recommendedId = '4.5 이상';
         for (const guide of tracheaSizeGuideCat) { if (diameter <= guide.diameter) { recommendedId = guide.id; break; } }
        resultText.textContent = recommendedId;
        resultContainer.classList.remove('hidden');
    }

    function saveCatEtTubeSelection() {
        const sizeInput = anesthesiaTab.querySelector('#cat_selectedEtTubeSize');
        if (!sizeInput || !sizeInput.value) { alert('최종 ET Tube 사이즈를 입력해주세요.'); sizeInput.focus(); return; }
        selectedCatTubeInfo.size = parseFloat(sizeInput.value);
        selectedCatTubeInfo.cuff = anesthesiaTab.querySelector('#cat_selectedEtTubeCuff').checked;
        selectedCatTubeInfo.notes = anesthesiaTab.querySelector('#cat_selectedEtTubeNotes').value;
        const saveButton = anesthesiaTab.querySelector('#saveCatEtTubeSelection');
    ...
    function updateCatTubeDisplay() {
        const displayDiv = anesthesiaTab.querySelector('#cat_et_tube_selection_display');
    ...
    anesthesiaTab.querySelectorAll('#patient_status, #renal_status, #chill_protocol').forEach(select => {
        select.addEventListener('change', calculateAnesthesiaAll);
    });
    
    const handoutTab = anesthesiaTab.querySelector('#norspanHandoutTab');
    if (handoutTab) {
        const attachDateEl = handoutTab.querySelector('#attachDate');
        const attachTimeEl = handoutTab.querySelector('#attachTime');
        if(attachDateEl && attachTimeEl){
            ...
            calculateNorspanRemovalDate();
            attachDateEl.addEventListener('change', calculateNorspanRemovalDate);
            attachTimeEl.addEventListener('change', calculateNorspanRemovalDate);
        }
        handoutTab.querySelector('.no-print button:nth-of-type(1)')?.addEventListener('click', () => window.print());
        handoutTab.querySelector('.no-print button:nth-of-type(2)')?.addEventListener('click', () => saveAnesthesiaImage('captureArea', '_통증패치_안내문'));
    }

    anesthesiaTab.querySelector('#calculate-trachea-btn')?.addEventListener('click', calculateTracheaSize);
    anesthesiaTab.querySelector('#trachea-input')?.addEventListener('input', calculateTracheaSize);
    anesthesiaTab.querySelector('#saveCatEtTubeSelection')?.addEventListener('click', saveCatEtTubeSelection);
    anesthesiaTab.querySelector('#prepTab .no-print button')?.addEventListener('click', exportPrepSheet);
...
### **5. 보호자 교육 대시보드 로직**
'보호자 교육' 탭의 환자 이름 연동, 탭 전환, 차트 생성, 계산, 이미지 저장 등 모든 기능을 담당합니다.

```javascript
// ===================================================================================
// ====================== [ 5. 보호자 교육 통합 대시보드 스크립트 ] ====================
// ===================================================================================
function setupEducationDashboard() {
    const educationTab = document.getElementById('content-education');
    if (!educationTab) return;

    const tabs = educationTab.querySelectorAll('.tab-button');
    const contents = educationTab.querySelectorAll('.tab-content');
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            const targetContent = educationTab.querySelector('#' + tab.dataset.tab + 'Content');
            if(targetContent) targetContent.classList.add('active');
        });
    });

    const globalNameInput = educationTab.querySelector('#globalPetName');
    if (globalNameInput) {
    ...
    function updateAllTitles() {
        ...
        const titles = {
            stomatitis: educationTab.querySelector('#stomatitisTitle'),
            cyclosporine: educationTab.querySelector('#cyclosporineTitle'),
            norspan: educationTab.querySelector('#norspanTitle'),
            gabapentin: educationTab.querySelector('#gabapentinTitle')
        };

        if (titles.stomatitis) titles.stomatitis.innerHTML = name ? `우리 ${name}${hasJongseong ? "이를" : "를"} 위한<br>만성 구내염 및 전발치 안내서` : `우리 아이를 위한<br>만성 구내염 및 전발치 안내서`;
        if (titles.cyclosporine) titles.cyclosporine.innerHTML = name ? `✨ ${name}${hasJongseong ? '이의' : '의'} 사이클로스포린 복약 안내문 ✨` : '✨ 사이클로스포린 복약 안내문 ✨';
        if (titles.norspan) titles.norspan.innerText = name ? `${name}${hasJongseong ? '이를' : '를'} 위한 통증 관리 패치 안내문` : '우리 아이를 위한 통증 관리 패치 안내문';
        if (titles.gabapentin) titles.gabapentin.innerHTML = name ? `<span>${name}</span><span>${hasJongseong ? '을' : '를'}</span> 위한 편안한 진료 준비 안내서` : `<span>우리 아이</span><span>를</span> 위한 편안한 진료 준비 안내서`;
    }

    const saveButtons = educationTab.querySelectorAll('#saveBtnTop, #saveBtnBottom');
    function saveActiveTabAsImage() {
        const activeTab = educationTab.querySelector('.tab-content.active');
        if (!activeTab) return;
        const petName = globalNameInput.value.trim() || '환자';
        const fileName = `${petName}_${activeTab.id.replace('Content', '')}_안내문.png`;
        
        html2canvas(activeTab, { scale: 2, useCORS: true, backgroundColor: '#ffffff' }).then(canvas => {
            const link = document.createElement('a');
            link.href = canvas.toDataURL('image/png');
            link.download = fileName;
    ...
    saveButtons.forEach(btn => btn.addEventListener('click', saveActiveTabAsImage));

    const ctx = educationTab.querySelector('#prognosisChart')?.getContext('2d');
    if (ctx) {
        new Chart(ctx, {
            type: 'doughnut',
            data: { labels: ['완전한 회복', '현저한 개선', '부분적 개선'], datasets: [{ data: [60, 25, 15], backgroundColor: ['rgba(52, 211, 153, 0.8)','rgba(251, 191, 36, 0.8)','rgba(239, 68, 68, 0.8)'], borderColor: ['#10b981','#f59e0b','#ef4444'], borderWidth: 2, hoverOffset: 10 }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { padding: 20 } } } }
        });
    }

    const attachDateInput = educationTab.querySelector('#norspanContent #attachDate');
    const attachTimeInput = educationTab.querySelector('#norspanContent #attachTime');
    
    function calculateNorspanRemovalDate() {
        const removalInfoDiv = educationTab.querySelector('#norspanContent #removalInfo');
        if (!attachDateInput || !attachTimeInput || !removalInfoDiv) return;
        if (!attachDateInput.value || !attachTimeInput.value) {
            removalInfoDiv.innerHTML = '<p class="font-bold text-yellow-900">날짜와 시간을 모두 입력해주세요.</p>'; return;
        }
        const attachDateTime = new Date(`${attachDateInput.value}T${attachTimeInput.value}`);
    ...
    if (attachDateInput && attachTimeInput) {
        attachDateInput.addEventListener('change', calculateNorspanRemovalDate);
        attachTimeInput.addEventListener('change', calculateNorspanRemovalDate);
        const now = new Date();
        now.setMinutes(now.getMinutes() - now.getTimezoneOffset());
    ...
    const weightInput = educationTab.querySelector('#petWeight');
    const durationInput = educationTab.querySelector('#duration');
    
    function calculateCycloDose(){
         const doseResultDiv = educationTab.querySelector('#doseResult');
         if(!weightInput || !doseResultDiv) return;
         const weight = parseFloat(weightInput.value);
         const duration = durationInput ? parseInt(durationInput.value) : 0;

        if (isNaN(weight) || weight <= 0) {
            doseResultDiv.innerHTML = '<p class="text-gray-700">👆 상단의 몸무게와 복용 기간을 입력하시면 자동으로 계산됩니다.</p>'; return;
        }
    ...
    if(weightInput && durationInput) {
        weightInput.addEventListener('input', calculateCycloDose);
        durationInput.addEventListener('input', calculateCycloDose);
    }
    
    // 초기 활성화
    const firstTabButton = educationTab.querySelector('.tab-button');
    if (firstTabButton) {
